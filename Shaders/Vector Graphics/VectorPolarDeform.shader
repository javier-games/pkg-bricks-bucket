Shader "Vector Graphics/Polar Deformed" {
    Properties {
        ...
        _Radious ("Radious", float) = 10
        _Length ("Length", Range(-1000,1000)) = 0
        _Angle ("Angle", Range(0.0,1.0)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            v2f vert (appdata v) {
                ...
                float wY = sin(time + v.vertex.y * _Frequency) * _Amplitude;
                float wX = cos(time + v.vertex.x * _Frequency) * _Amplitude;
                v.vertex.xyz = float3(v.vertex.x, v.vertex.y + wX, v.vertex.z);
               
                float r = _Radious + v.vertex.x;
                float th = (v.vertex.y/_Length) * 1.5708 - 6.2832 * _Angle;
                v.vertex.xyz = float3(r * cos(th), r * sin(th), v.vertex.z);
