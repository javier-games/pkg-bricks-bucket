...
/// If scene is valid, provides basic buttons to interact with the scene's role
/// in Build Settings.
/// </summary>
...
public class SceneReferencePropertyDrawer: PropertyDrawer {
    ...
    private void DrawSceneInfoGUI (Rect position, BuildUtils.BuildScene buildScene, int sceneControlID) {
        ...
        using (new EditorGUI.DisabledScope (readOnly)) {
            ...
            if (buildScene.buildIndex == -1) {
                ...
                Texture2D iconAdd = (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/Scripts/Framework/Utils/Editor/Add_ICN.png", typeof (Texture2D));
                if (iconAdd == null) {
                    ...
                    if (GUI.Button (buttonRect, iconAdd))
                        BuildUtils.AddBuildScene (buildScene);
                }
                buttonRect.width /= 2;
            ...
            // In build settings
                ...
                Texture2D icon = !isEnabled ? 
                (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/Scripts/Framework/Utils/Editor/Disable_ICN.png", typeof (Texture2D)) :
                (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/Scripts/Framework/Utils/Editor/Enable_ICN.png", typeof (Texture2D));
                if (icon == null) {
                ...
                Texture2D iconRemove = (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/Scripts/Framework/Utils/Editor/Remove_ICN.png", typeof (Texture2D));
                if (iconRemove == null) {
                    ...
                    if (GUI.Button(buttonRect,iconRemove))
                        BuildUtils.RemoveBuildScene (buildScene);
                }
            }
        }
        ...
        Texture2D iconSettings = (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/Scripts/Framework/Utils/Editor/Settings_ICN.png", typeof (Texture2D));
        if (iconSettings == null) {
            ...
            if (GUI.Button(buttonRect, iconSettings)) {
                BuildUtils.OpenBuildSettings ();
            }
        }
    ...
    private static class DrawUtils {
        /// <summary>
        /// Draw a GUI button, choosing between a short and a long button text
        /// based on if it fits
        /// </summary>
    ...
    static private class BuildUtils {
        // time in seconds that we have to wait before we query again when
        ...
        /// Caches value and only queries state a max of every 'minCheckWait'
        /// seconds.
        /// </summary>
        ...
        /// <summary>
        /// A blocking call to the Version Control system to see if the build
        /// settings asset is readonly.
        /// Use BuildSettingsIsReadOnly for version that caches the value for
        ...
        static private bool QueryBuildSettingsStatus () {
            ...
            // If offline (and are using a version control provider that
            // requires checkout) we cannot edit.
        ...
        /// <summary>
        /// For a given Scene Asset object reference, extract its build settings
        /// data, including buildIndex.
        /// </summary>
        ...
        /// <summary>
        /// Display Dialog to remove a scene from build settings (or just
        /// disable it)
        /// </summary>
